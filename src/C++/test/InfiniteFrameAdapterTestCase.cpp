...
std::string finishFix(std::string beginString, std::string body) {
  std::string message = "8=" + std::move(beginString) + "\0019=" + std::to_string(body.size()) + "\001"
                        + std::move(body) + "10=000\001";
...
struct CallbackContext {
  ...
  irfq_infinite_status_v1 crossWaitStatus{IRFQ_INFINITE_STATUS_OK_V1};
  bool fenced{false};
  bool released{false};
  ...
  irfq_infinite_operation_response_v1 bootstrapCloseResponse{};
  irfq_infinite_operation_response_v1 fenceCloseResponse{};
  bool enforceWaitTokenOrder{false};
  irfq_infinite_handle_v1 lowerWaitToken{};
  irfq_infinite_handle_v1 laterWaitToken{};
  bool laterWaitEntered{false};
  bool lowerRetired{false};
  bool laterWaitTimedOut{false};
  ...
  bool duplicateBootstrapHandle{false};
  ...
  irfq_infinite_status_v1 handoffCloseStatus{IRFQ_INFINITE_STATUS_OK_V1};
  irfq_infinite_status_v1 handoffCrossStatus{IRFQ_INFINITE_STATUS_OK_V1};
...
CloseResult nestedCloseResult(irfq_infinite_handle_v1 connection, std::uint32_t reason = 1) {
  ...
  const auto status = irfq_infinite_connection_close_v1(connection, &request, &response, sizeof(response));
  return {status, response};
}

irfq_infinite_status_v1 nestedClose(irfq_infinite_handle_v1 connection, std::uint32_t reason = 1) {
  return nestedCloseResult(connection, reason).status;
}

irfq_infinite_status_v1 nestedWait(irfq_infinite_handle_v1 connection, irfq_infinite_handle_v1 token) {
...
irfq_infinite_status_v1 bootstrapCallback(
  ...
  response.header.status = IRFQ_INFINITE_STATUS_OK_V1;
  {
    ...
    response.connection = context.duplicateBootstrapHandle
                              ? irfq_infinite_handle_v1{UINT64_C(7), UINT64_C(9)}
                              : irfq_infinite_handle_v1{context.nextConnection++, UINT64_C(9)};
  ...
  if (context.closeCrossConnectionOnBootstrap) {
    const auto closed = nestedCloseResult(context.crossConnection, UINT32_C(71));
    context.bootstrapCloseStatus = closed.status;
    context.bootstrapCloseResponse = closed.response;
  }
...
irfq_infinite_status_v1 registerCallback(
  ...
  if (context.callCrossWait) {
    context.crossWaitStatus = nestedWait(context.crossConnection, context.crossToken);
  }
  ...
  if (context.concurrentCrossCloses && registrationCallbackDepth == 1) {
    ...
    const auto closed
        = nestedCloseResult(first ? context.secondAdapterConnection : context.firstAdapterConnection, first ? 41 : 42);
    {
      std::unique_lock<std::mutex> lock(context.mutex);
      if (first) {
        context.firstCloseStatus = closed.status;
        context.firstCloseResponse = closed.response;
      } else {
        context.secondCloseStatus = closed.status;
        context.secondCloseResponse = closed.response;
      }
      ++context.concurrentNestedCompleted;
      context.condition.notify_all();
      context.condition.wait(lock, [&context]() { return context.concurrentNestedCompleted == 2; });
  ...
  const auto writtenLength = response->header.status == IRFQ_INFINITE_STATUS_OK_V1
                                 ? responseBytes
                                 : sizeof(irfq_infinite_dispatch_response_v1);
  std::memcpy(outputBuffer, stagedBytes, writtenLength);
  static_cast<irfq_infinite_output_header_v1 *>(outputBuffer)->written_length = writtenLength;
  return response->header.status;
}
...
irfq_infinite_status_v1 waitCallback(
  ...
  auto &context = *static_cast<CallbackContext *>(opaque);
  {
    ...
    context.waitEntered = true;
    context.condition.notify_all();
    if (context.enforceWaitTokenOrder && request->token.object == context.laterWaitToken.object
        && request->token.generation == context.laterWaitToken.generation) {
      context.laterWaitEntered = true;
      context.condition.notify_all();
      if (!context.condition.wait_for(lock, std::chrono::seconds(2), [&context]() { return context.lowerRetired; })) {
        context.laterWaitTimedOut = true;
      }
    }
    context.condition.wait(lock, [&context]() { return !context.blockWait || context.allowWait; });
  }
...
irfq_infinite_status_v1 fenceCallback(void *opaque, irfq_infinite_handle_v1, std::uint32_t) {
  ...
  context.condition.notify_all();
  context.fenceCompletedOrder.store(++context.completionOrder, std::memory_order_release);
  if (context.fenceThrows) {
    throw std::runtime_error("fence callback failure");
  }
  if (context.closeSameHandleFromFence) {
    const auto closed = nestedCloseResult(context.crossConnection, UINT32_C(72));
    context.fenceCloseStatus = closed.status;
    context.fenceCloseResponse = closed.response;
  }
...
irfq_infinite_status_v1 releaseCallback(void *opaque, irfq_infinite_handle_v1 externalConnection, std::uint32_t) {
  ...
  context.releaseCompletedOrder.store(++context.completionOrder, std::memory_order_release);
...
irfq_infinite_status_v1 handoffCallback(
  ...
  {
    std::unique_lock<std::mutex> lock(context.mutex);
    context.handoffEntered = true;
    context.condition.notify_all();
    context.condition.wait(lock, [&context]() { return !context.blockHandoff || context.allowHandoff; });
  }
  if (context.closeSameHandleOnHandoff) {
    context.handoffCloseStatus = nestedClose(context.adapterConnection, UINT32_C(74));
  }
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][candidate-claim-apply]") {
  ...
  SECTION("apply retirement remains final while handoff is in progress") {
    ownerContext.blockHandoff = true;
    irfq_infinite_status_v1 applyStatus = IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1;
    std::thread applying([&]() { applyStatus = applyClassification(owner.response.connection, classified.second); });
    {
      std::unique_lock<std::mutex> lock(ownerContext.mutex);
      ownerContext.condition.wait(lock, [&ownerContext]() { return ownerContext.handoffEntered; });
    }
    ...
    {
      std::lock_guard<std::mutex> lock(ownerContext.mutex);
      ownerContext.allowHandoff = true;
    }
  ...
  SECTION("same-handle close during handoff fails closed without blocking") {
    ownerContext.adapterConnection = owner.response.connection;
    ownerContext.closeSameHandleOnHandoff = true;
    CHECK(applyClassification(owner.response.connection, classified.second) == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
    CHECK(ownerContext.handoffCloseStatus == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
    CHECK(ownerContext.handoffCount == 1);
    CHECK(ownerContext.fenceCount == 1);
  }

  SECTION("sibling operation during handoff fails closed without blocking") {
    const auto sibling = bootstrapConnection(ownerEngine.engine, "CLAIMAPPLYSIBLING", UINT64_C(2074));
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][callback-close-fence-reentry]") {
  ...
  REQUIRE(origin.status == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
  CHECK(origin.response.header.status == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
  CHECK(origin.response.outcome == IRFQ_INFINITE_BOOTSTRAP_FENCED_V1);

  CHECK(context.bootstrapCloseStatus == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
  CHECK(context.bootstrapCloseResponse.header.status == context.bootstrapCloseStatus);
  CHECK(context.bootstrapCloseResponse.header.written_length == sizeof(context.bootstrapCloseResponse));
  CHECK(context.bootstrapCloseResponse.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
  CHECK(context.fenceCloseStatus == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
  CHECK(context.fenceCloseResponse.header.status == context.fenceCloseStatus);
  CHECK(context.fenceCloseResponse.header.written_length == sizeof(context.fenceCloseResponse));
  CHECK(context.fenceCloseResponse.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
  CHECK(context.fenceCount == 2);
  CHECK(context.releaseCount == 1);
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][fence-quiescence]") {
  ...
  CHECK(context.fenceCompletedOrder.load(std::memory_order_acquire) < closeCompletedOrder);
  CHECK(context.fenceCompletedOrder.load(std::memory_order_acquire) < shutdownCompletedOrder);
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][bootstrap-capacity]") {
  ...
  std::size_t attemptsStarted = 0;
  std::size_t attemptsCompleted = 0;
  ...
  {
    std::unique_lock<std::mutex> lock(context.mutex);
    capacityReached = context.condition.wait_for(lock, std::chrono::seconds(10), [&]() {
      return attemptsStarted == attempts && context.bootstrapEntered == IRFQ_INFINITE_MAX_CONNECTIONS_V1
             && attemptsCompleted >= 1;
    });
    if (!capacityReached) {
      context.allowBootstrap = true;
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][callback-ancestry-wait]") {
#ifdef CLOCK_TAI
  CallbackContext context;
  const auto initialized = initializeEngine(context);
  const auto first = bootstrapConnection(initialized.engine, "ANCESTRYWAITA", UINT64_C(8150));
  const auto second = bootstrapConnection(initialized.engine, "ANCESTRYWAITB", UINT64_C(8160));
  REQUIRE(first.status == IRFQ_INFINITE_STATUS_OK_V1);
  REQUIRE(second.status == IRFQ_INFINITE_STATUS_OK_V1);

  const auto secondFrame = finishFix("35=0\00134=2\00149=ANCESTRYWAITB\00156=VENUE\00152=20260826-08:08:09.000\001");
  const auto secondRegistration = dispatchFrame(second.response.connection, secondFrame);
  REQUIRE(secondRegistration.status == IRFQ_INFINITE_STATUS_OK_V1);

  context.crossConnection = second.response.connection;
  context.crossToken = secondRegistration.registration.token;
  context.callCrossWait = true;
  const auto firstFrame = finishFix("35=0\00134=2\00149=ANCESTRYWAITA\00156=VENUE\00152=20260826-08:08:09.000\001");
  CHECK(dispatchFrame(first.response.connection, firstFrame).status == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
  CHECK(context.crossWaitStatus == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
  CHECK(context.waitCallCount == 0);

  context.callCrossWait = false;
  closeConnection(first.response.connection);
  closeConnection(second.response.connection);
  shutdownEngine(initialized.engine);
#endif
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][callback-close-cycle]") {
  ...
  CHECK(context.firstCloseResponse.header.status == context.firstCloseStatus);
  CHECK(context.secondCloseResponse.header.status == context.secondCloseStatus);
  CHECK(context.firstCloseResponse.header.written_length == sizeof(context.firstCloseResponse));
  CHECK(context.secondCloseResponse.header.written_length == sizeof(context.secondCloseResponse));
  CHECK(context.firstCloseResponse.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
  CHECK(context.secondCloseResponse.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][fifo-wait]") {
#ifdef CLOCK_TAI
  CallbackContext context;
  context.enforceWaitTokenOrder = true;
  context.lowerWaitToken = {context.nextToken, UINT64_C(1)};
  context.laterWaitToken = {context.nextToken + 1, UINT64_C(1)};
  const auto initialized = initializeEngine(context);
  const auto bootstrapped = bootstrapConnection(initialized.engine, "FIFOWAIT", UINT64_C(9350));
  REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);

  const auto lower = finishFix("35=0\00134=2\00149=FIFOWAIT\00156=VENUE\00152=20260826-08:08:09.000\001");
  const auto later = finishFix("35=0\00134=3\00149=FIFOWAIT\00156=VENUE\00152=20260826-08:08:10.000\001");
  const auto batch = lower + later;
  const irfq_infinite_dispatch_request_v1 dispatch{
      sizeof(dispatch),
      IRFQ_INFINITE_FRAME_ADAPTER_ABI_VERSION_V1,
  ...
  SECTION("lower retirement advances later wait") {
    irfq_infinite_status_v1 laterWaitStatus = IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1;
    std::thread laterWait([&]() { laterWaitStatus = waitAtHead(bootstrapped.response.connection, laterToken); });
    {
      std::unique_lock<std::mutex> lock(context.mutex);
      context.condition.wait_for(lock, std::chrono::milliseconds(500), [&context]() {
        return context.laterWaitEntered;
      });
    }

    irfq_infinite_status_v1 lowerWaitStatus = IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1;
    irfq_infinite_status_v1 lowerClassifyStatus = IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1;
    irfq_infinite_status_v1 lowerApplyStatus = IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1;
    std::thread lowerRetirement([&]() {
      lowerWaitStatus = waitAtHead(bootstrapped.response.connection, lowerToken);
      if (lowerWaitStatus == IRFQ_INFINITE_STATUS_AT_HEAD_V1) {
        const auto classified = classifyAtHead(bootstrapped.response.connection, lowerToken);
        lowerClassifyStatus = classified.first;
        if (lowerClassifyStatus == IRFQ_INFINITE_STATUS_CLASSIFIED_V1) {
          lowerApplyStatus = applyClassification(bootstrapped.response.connection, classified.second);
        }
      }
      {
        std::lock_guard<std::mutex> lock(context.mutex);
        context.lowerRetired = true;
      }
      context.condition.notify_all();
    });

    lowerRetirement.join();
    laterWait.join();
    CHECK_FALSE(context.laterWaitTimedOut);
    CHECK(lowerWaitStatus == IRFQ_INFINITE_STATUS_AT_HEAD_V1);
    CHECK(lowerClassifyStatus == IRFQ_INFINITE_STATUS_CLASSIFIED_V1);
    CHECK(lowerApplyStatus == IRFQ_INFINITE_STATUS_APPLIED_V1);
    CHECK(laterWaitStatus == IRFQ_INFINITE_STATUS_AT_HEAD_V1);
    CHECK(context.waitCallCount == 2);
  }

  SECTION("close wakes a locally waiting token") {
    std::atomic<bool> waitStarting{false};
    CloseResult closed{IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1, {}};
    std::thread closing([&]() {
      while (!waitStarting.load(std::memory_order_acquire)) {
        std::this_thread::yield();
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
      closed = nestedCloseResult(bootstrapped.response.connection, UINT32_C(93));
    });
    const irfq_infinite_head_request_v1 head{sizeof(head), IRFQ_INFINITE_FRAME_ADAPTER_ABI_VERSION_V1, laterToken, {}};
    auto waited = output<irfq_infinite_operation_response_v1>();
    waitStarting.store(true, std::memory_order_release);
    const auto waitStatus
        = irfq_infinite_connection_wait_head_v1(bootstrapped.response.connection, &head, &waited, sizeof(waited));
    closing.join();
    CHECK(waitStatus == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
    CHECK(waited.header.status == waitStatus);
    CHECK(waited.header.written_length == sizeof(waited));
    CHECK(waited.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
    CHECK(context.waitCallCount == 0);
    CHECK(closed.status == IRFQ_INFINITE_STATUS_CLOSED_V1);
    CHECK(closed.response.header.status == closed.status);
    CHECK(closed.response.header.written_length == sizeof(closed.response));
    CHECK(closed.response.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSED_V1);
  }
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][callback-shutdown-cycle]") {
  ...
  std::array<irfq_infinite_status_v1, 2> statuses{};
  std::thread firstDispatch([&]() { statuses[0] = dispatchFrame(first.response.connection, firstFrame).status; });
  std::thread secondDispatch([&]() { statuses[1] = dispatchFrame(second.response.connection, secondFrame).status; });
  firstDispatch.join();
  secondDispatch.join();
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][fixt-application-dictionaries]") {
#ifdef CLOCK_TAI
  const auto exercise = [](const std::string &sender, const std::string &applicationVersion, bool approved) {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto logon = finishFix(
        "FIXT.1.1",
        "35=A\00134=1\00149=" + sender + "\00156=VENUE\00152=20260826-08:08:08.000\00198=0\001108=30\0011137=7\001");
    const auto bootstrapped = bootstrapFrame(initialized.engine, logon, UINT64_C(11720));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);

    const auto application = finishFix(
        "FIXT.1.1",
        "35=D\00134=2\00149=" + sender + "\00156=VENUE\00152=20260826-08:08:09.000\0011128=" + applicationVersion
            + "\00154=1\00138=1\00140=1\001");
    const auto registration = dispatchFrame(bootstrapped.response.connection, application);
    REQUIRE(registration.status == IRFQ_INFINITE_STATUS_OK_V1);
    REQUIRE(
        waitAtHead(bootstrapped.response.connection, registration.registration.token)
        == IRFQ_INFINITE_STATUS_AT_HEAD_V1);
    const auto classified = classifyAtHead(bootstrapped.response.connection, registration.registration.token);
    if (approved) {
      REQUIRE(classified.first == IRFQ_INFINITE_STATUS_CLASSIFIED_V1);
      CHECK(classified.second.action == IRFQ_INFINITE_ACTION_PROTOCOL_DISPOSITION_V1);
      CHECK(
          applyClassification(bootstrapped.response.connection, classified.second) == IRFQ_INFINITE_STATUS_APPLIED_V1);
    } else {
      CHECK(classified.first == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
      CHECK(classified.second.action == IRFQ_INFINITE_ACTION_FAILURE_V1);
    }
    closeConnection(bootstrapped.response.connection);
    shutdownEngine(initialized.engine);
  };

  SECTION("an approved per-message override uses its installed application dictionary") {
    exercise("FIXTOVERRIDE", "6", true);
  }
  SECTION("an unsupported per-message override cannot fall back to an empty dictionary") {
    exercise("FIXTUNKNOWN", "99", false);
  }
#endif
}
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][bootstrap-cleanup-failure]") {
  const auto exercise = [](bool releaseFailure) {
    ...
    CHECK(context.fenceCount == 1);
    CHECK(context.releaseCount == (releaseFailure ? 1 : 0));
  ...
  SECTION("terminal shutdown still drains an existing connection") {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto live = bootstrapConnection(initialized.engine, "TERMINALDRAINLIVE", UINT64_C(11771));
    REQUIRE(live.status == IRFQ_INFINITE_STATUS_OK_V1);

    context.bootstrapReservedNonzero = true;
    context.fenceAcknowledgement = IRFQ_INFINITE_STATUS_OK_V1;
    const auto failed = bootstrapConnection(initialized.engine, "TERMINALDRAINFAIL", UINT64_C(11773));
    REQUIRE(failed.status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    REQUIRE(context.fenceCount == 1);
    REQUIRE(context.releaseCount == 0);
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][duplicate-bootstrap-owner]") {
  CallbackContext context;
  context.duplicateBootstrapHandle = true;
  const auto initialized = initializeEngine(context);
  const auto owner = bootstrapConnection(initialized.engine, "DUPLICATEOWNER", UINT64_C(11772));
  REQUIRE(owner.status == IRFQ_INFINITE_STATUS_OK_V1);

  const auto duplicate = bootstrapConnection(initialized.engine, "DUPLICATEALIAS", UINT64_C(11774));
  CHECK(duplicate.status == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
  CHECK(context.fenceCount == 1);
  CHECK(context.releaseCount == 0);

  CHECK(nestedClose(owner.response.connection) == IRFQ_INFINITE_STATUS_CLOSED_V1);
  CHECK(context.fenceCount == 1);
  CHECK(context.releaseCount == 1);
  shutdownEngine(initialized.engine);
}

TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][publication-closure-lifecycle]") {
  SECTION("an alias rejected during session construction cannot release its live owner") {
    CallbackContext context;
    context.duplicateBootstrapHandle = true;
    const auto initialized = initializeEngine(context);
    const auto owner = bootstrapConnection(initialized.engine, "SESSIONALIAS", UINT64_C(11780));
    REQUIRE(owner.status == IRFQ_INFINITE_STATUS_OK_V1);

    const auto alias = bootstrapConnection(initialized.engine, "SESSIONALIAS", UINT64_C(11782));
    CHECK(alias.status == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
    CHECK(context.fenceCount == 1);
    CHECK(context.releaseCount == 0);
  ...
  SECTION("a failed raw fence acknowledgement forbids release") {
    ...
    context.fenceAcknowledgement = IRFQ_INFINITE_STATUS_OK_V1;
    const auto initialized = initializeEngine(context);
    const auto rejected = bootstrapConnection(initialized.engine, "RAWFENCEFAIL", UINT64_C(11784));
    CHECK(rejected.status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(context.fenceCount == 1);
    CHECK(context.releaseCount == 0);
  }
...
#ifdef CLOCK_TAI
  SECTION("same-handle callback reentry publishes the failed fence acknowledgement") {
    CallbackContext context;
    context.reenterSameHandle = true;
    context.fenceAcknowledgement = IRFQ_INFINITE_STATUS_OK_V1;
    const auto initialized = initializeEngine(context);
    const auto bootstrapped = bootstrapConnection(initialized.engine, "REENTRYACK", UINT64_C(11786));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);
    context.adapterConnection = bootstrapped.response.connection;

    const auto frame = finishFix("35=0\00134=2\00149=REENTRYACK\00156=VENUE\00152=20260826-08:08:09.000\001");
    CHECK(dispatchFrame(bootstrapped.response.connection, frame).status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(context.sameHandleReentryStatus == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(context.fenceCount == 1);
    CHECK(context.releaseCount == 0);
  }

  SECTION("a failed fence does not wait for a blocked head callback") {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto bootstrapped = bootstrapConnection(initialized.engine, "FENCEWAIT", UINT64_C(11788));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);
    const auto frame = finishFix("35=0\00134=2\00149=FENCEWAIT\00156=VENUE\00152=20260826-08:08:09.000\001");
    const auto registration = dispatchFrame(bootstrapped.response.connection, frame);
    REQUIRE(registration.status == IRFQ_INFINITE_STATUS_OK_V1);

    context.blockWait = true;
    irfq_infinite_status_v1 waitStatus = IRFQ_INFINITE_STATUS_OK_V1;
    std::thread waiting(
        [&]() { waitStatus = waitAtHead(bootstrapped.response.connection, registration.registration.token); });
    {
      std::unique_lock<std::mutex> lock(context.mutex);
      context.condition.wait(lock, [&context]() { return context.waitEntered; });
    }
    ...
    std::thread closing([&]() {
      closeStatus = nestedClose(bootstrapped.response.connection, UINT32_C(81));
      std::lock_guard<std::mutex> lock(context.mutex);
      closeCompleted = true;
      context.condition.notify_all();
    });

    bool returnedBeforeWaitRelease = false;
    {
      std::unique_lock<std::mutex> lock(context.mutex);
      returnedBeforeWaitRelease
          = context.condition.wait_for(lock, std::chrono::seconds(2), [&closeCompleted]() { return closeCompleted; });
      context.allowWait = true;
      context.condition.notify_all();
    }
    waiting.join();
    closing.join();

    CHECK(returnedBeforeWaitRelease);
    CHECK(closeStatus == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(waitStatus == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(context.fenceCount == 1);
    CHECK(context.releaseCount == 0);
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][parser-credential-wipe]") {
  ...
  SECTION("discard erases a retained partial credential frame") {
    FIX::InfiniteCompleteFrameDispatcher dispatcher({1, IRFQ_INFINITE_MAX_FRAME_BYTES_V1});
    const auto partial = dispatcher.process(retained.data(), retained.size(), []() { return INT64_C(1); });
    REQUIRE_FALSE(partial.terminalFault);
    REQUIRE(FIX::InfiniteCompleteFrameTestAccess::parserBufferContains(dispatcher, credential));
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][failed-fence-publication]") {
#ifdef CLOCK_TAI
  SECTION("terminal dispatch") {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto bootstrapped = bootstrapConnection(initialized.engine, "FENCEPUBDISPATCH", UINT64_C(11781));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);
    context.fenceAcknowledgement = IRFQ_INFINITE_STATUS_OK_V1;

    const auto dispatched = dispatchFrame(bootstrapped.response.connection, "X");
    CHECK(dispatched.status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(context.fenceCount == 1);
    shutdownEngine(initialized.engine);
  }
  ...
  SECTION("head wait") {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto bootstrapped = bootstrapConnection(initialized.engine, "FENCEPUBWAIT", UINT64_C(11783));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);
    const auto frame = finishFix("35=0\00134=2\00149=FENCEPUBWAIT\00156=VENUE\00152=20260826-08:08:09.000\001");
    ...
    const irfq_infinite_head_request_v1 request{
        sizeof(request),
        IRFQ_INFINITE_FRAME_ADAPTER_ABI_VERSION_V1,
        dispatched.registration.token,
        {}};
    auto response = output<irfq_infinite_operation_response_v1>();
    CHECK(
        irfq_infinite_connection_wait_head_v1(bootstrapped.response.connection, &request, &response, sizeof(response))
        == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(response.header.status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(response.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
    CHECK(context.fenceCount == 1);
  ...
  SECTION("classification") {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto bootstrapped = bootstrapConnection(initialized.engine, "FENCEPUBCLASSIFY", UINT64_C(11785));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);
    const auto frame = finishFix("35=0\00134=2\00149=FENCEPUBCLASSIFY\00156=VENUE\00152=20260826-08:08:09.000\001");
    const auto dispatched = dispatchFrame(bootstrapped.response.connection, frame);
    REQUIRE(dispatched.status == IRFQ_INFINITE_STATUS_OK_V1);
    REQUIRE(
    ...
    CHECK(classified.second.header.status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(context.fenceCount == 1);
    shutdownEngine(initialized.engine);
  }
  ...
  SECTION("application") {
    CallbackContext context;
    const auto initialized = initializeEngine(context);
    const auto bootstrapped = bootstrapConnection(initialized.engine, "FENCEPUBAPPLY", UINT64_C(11787));
    REQUIRE(bootstrapped.status == IRFQ_INFINITE_STATUS_OK_V1);
    const auto frame = finishFix("35=0\00134=2\00149=FENCEPUBAPPLY\00156=VENUE\00152=20260826-08:08:09.000\001");
    const auto dispatched = dispatchFrame(bootstrapped.response.connection, frame);
    REQUIRE(dispatched.status == IRFQ_INFINITE_STATUS_OK_V1);
    REQUIRE(
        waitAtHead(bootstrapped.response.connection, dispatched.registration.token) == IRFQ_INFINITE_STATUS_AT_HEAD_V1);
    ...
    const irfq_infinite_apply_request_v1 request{
        sizeof(request),
        IRFQ_INFINITE_FRAME_ADAPTER_ABI_VERSION_V1,
        {classified.second.classification.object + 1, classified.second.classification.generation},
        classified.second.authorization,
        {}};
    auto response = output<irfq_infinite_operation_response_v1>();
    CHECK(
        irfq_infinite_connection_apply_v1(bootstrapped.response.connection, &request, &response, sizeof(response))
        == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(response.header.status == IRFQ_INFINITE_STATUS_INTERNAL_ERROR_V1);
    CHECK(response.lifecycle == IRFQ_INFINITE_CONNECTION_CLOSING_V1);
    CHECK(context.fenceCount == 1);
...
TEST_CASE("InfiniteFrameAdapterTests", "[infinite][adapter][lifecycle-callback-failure]") {
  const auto exercise = [](bool releaseFailure) {
    ...
    if (releaseFailure) {
      ...
      CHECK(replacement.status == IRFQ_INFINITE_STATUS_STREAM_FENCED_V1);
      CHECK(replacementContext.fenceCount == 1);
