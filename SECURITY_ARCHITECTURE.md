...
vendor bytes, unresolved linkage or incomplete package metadata fail the
inventory step. Catch2 is source/test-only and is excluded from the installed
library inventory. Optional database/binding profiles are not covered by this
TLS build's runtime inventory.

The vendored `double-conversion` snapshot is a modified 2.0.1-derived fork,
not an unmodified release: it includes local trimming/portability changes.
The upstream version is used conservatively for advisory matching; the digest
identifies the actual bytes. `scope_guard` matches v1.1.0 apart from the added
licence comment and final newline. Scanner matches require triage, and a clean
upstream advisory lookup does not review local vendor modifications. Native
Grype CPE generation enables advisory matching for the generic C++ entries.
